...
export async function generateAndSendMorningReport(): Promise<void> {
  ...
  // ── 4. New CAMP-BLAST enrollments since 5 PM ET yesterday ──────────────────
  // 5 PM ET = 21:00 UTC (EDT) or 22:00 UTC (EST). Using 21:00 UTC (conservative).
  // Scoped to the summer_camp_blast tag: this is the Camp Blast report, so it must
  // only count camp leads. A site-wide count pulled in non-camp enrollments (e.g. a
  // Little Tigers trial bumped to enrolled), which made the camp number wrong.
  ...
  const [enrolledResult] = await db.execute(
    sql`SELECT COUNT(*) as cnt FROM leads
        WHERE pipelineStage = 'enrolled'
          ...
          AND tags LIKE '%summer_camp_blast%'`
  ) as unknown as [Array<{ cnt: number }>];
