...
import { memo, type ReactNode, type ComponentProps, type ComponentType } from "react";
...
const components: Record<string, ComponentType<Record<string, unknown> & ExtraProps>> = {
  // Headings - using tracking-tight for Vercel-style typography
  h1: ({ children }: { children?: ReactNode; [key: string]: unknown }) => (
    <h1 className="text-3xl font-semibold tracking-tight mt-8 mb-4 first:mt-0">
      {children}
    </h1>
  ),
  h2: ({ children }: { children?: ReactNode; [key: string]: unknown }) => (
    <h2 className="text-2xl font-semibold tracking-tight mt-8 mb-3 first:mt-0">
      {children}
    </h2>
  ),
  h3: ({ children }: { children?: ReactNode; [key: string]: unknown }) => (
    <h3 className="text-xl font-semibold tracking-tight mt-6 mb-3 first:mt-0">
      {children}
    </h3>
  ),
  h4: ({ children }: { children?: ReactNode; [key: string]: unknown }) => (
    <h4 className="text-lg font-semibold mt-6 mb-2 first:mt-0">{children}</h4>
  ),

  // Text elements
  p: ({ children }: { children?: ReactNode; [key: string]: unknown }) => (
    <p className="mb-4 leading-7 last:mb-0">{children}</p>
  ),
  a: ({ href, children }: { href?: string; children?: ReactNode; [key: string]: unknown }) => (
    <a
      href={href}
      target="_blank"
  ...
  strong: ({ children }: { children?: ReactNode; [key: string]: unknown }) => (
    <strong className="font-semibold">{children}</strong>
  ),
  em: ({ children }: { children?: ReactNode; [key: string]: unknown }) => (
    <em className="italic">{children}</em>
  ),

  // Lists
  ul: ({ children }: { children?: ReactNode; [key: string]: unknown }) => (
    <ul className="list-disc pl-6 mb-4 space-y-2">{children}</ul>
  ),
  ol: ({ children }: { children?: ReactNode; [key: string]: unknown }) => (
    <ol className="list-decimal pl-6 mb-4 space-y-2">{children}</ol>
  ),
  li: ({ children }: { children?: ReactNode; [key: string]: unknown }) => (
    <li className="leading-7">{children}</li>
  ),

  // Block elements
  blockquote: ({ children }: { children?: ReactNode; [key: string]: unknown }) => (
    <blockquote className="border-l-2 border-muted-foreground/30 pl-4 italic text-muted-foreground my-4">
      {children}
    </blockquote>
  ),
  hr: (_props: Record<string, unknown>) => <hr className="border-border my-8" />,

  // Tables - with responsive wrapper
  table: ({ children }: { children?: ReactNode; [key: string]: unknown }) => (
    <div className="overflow-x-auto my-4">
      <table className="w-full border-collapse text-sm">{children}</table>
    </div>
  ),
  thead: ({ children }: { children?: ReactNode; [key: string]: unknown }) => <thead>{children}</thead>,
  tbody: ({ children }: { children?: ReactNode; [key: string]: unknown }) => <tbody>{children}</tbody>,
  tr: ({ children }: { children?: ReactNode; [key: string]: unknown }) => (
    <tr className="border-b border-border">{children}</tr>
  ),
  th: ({ children }: { children?: ReactNode; [key: string]: unknown }) => (
    <th className="border border-border bg-muted px-4 py-2 text-left font-semibold">
      {children}
    </th>
  ),
  td: ({ children }: { children?: ReactNode; [key: string]: unknown }) => (
    <td className="border border-border px-4 py-2">{children}</td>
  ),

  // Media
  img: ({ src, alt }: { src?: string; alt?: string; [key: string]: unknown }) => (
