...
function sourceLabel(source?: string | null): string {
  return SOURCE_SECTIONS.find(s => s.key === source)?.label ?? OTHER.label;
}
function bucketOf(source?: string | null): string {
  return SOURCE_SECTIONS.some(s => s.key === source) ? String(source) : OTHER.key;
}
...
// Signed waivers "on file". Search across all, grouped into sections by source.
export default function WaiversView() {
  ...
  // Group filtered waivers by source, preserving the SOURCE_SECTIONS order + Other last.
  const groups = useMemo(() => {
    const map: Record<string, any[]> = {};
    for (const w of filtered) {
      const b = bucketOf(w.source);
      (map[b] ??= []).push(w);
    }
    const ordered = [...SOURCE_SECTIONS, OTHER]
      .map(s => ({ ...s, rows: map[s.key] ?? [] }))
      ...
      {list.isLoading ? (
        ...
        <>
          ...
          <div className="relative">
            ...
            <Input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search by student, parent, email, or phone" className="pl-9" />
            ...
            groups.map(group => (
              <div key={group.key} className="space-y-2">
                <div className="flex items-center gap-2 px-0.5">
                  <h3 className="text-xs font-bold uppercase tracking-wider text-gray-400">{group.label}</h3>
                  <span className="text-[11px] font-semibold text-gray-400 bg-gray-100 rounded-full px-2 py-0.5">{group.rows.length}</span>
                </div>
                {group.rows.map((w: any) => (
                  <WaiverRow key={w.id} w={w} onOpen={() => setOpen(w)} onRemove={() => remove(w)} />
                ))}
              </div>
            ))
          )}
...
function WaiverRow({ w, onOpen, onRemove }: { w: any; onOpen: () => void; onRemove: () => void }) {
  const kids = parseArr(w.students);
  return (
    <div className="bg-white border border-gray-200 rounded-lg p-3.5 hover:border-[#1a2d5a]/40 transition-colors flex items-center justify-between gap-3">
      ...
      <button onClick={onRemove} title="Delete waiver" className="text-gray-300 hover:text-[#c41e3a] shrink-0">
