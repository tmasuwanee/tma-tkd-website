...
// Fall Break is a single fixed week, no week selection needed
        ...
        <CardContent className="pt-6 grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div className="sm:col-span-2 flex items-start gap-3 p-3 bg-gray-50 border border-gray-200 rounded">
            <Checkbox
              ...
              className="mt-0.5"
            />
            <Label htmlFor="smsConsentFall" className="text-xs text-gray-700 leading-relaxed cursor-pointer font-normal">
...
// Step 3: Program Selection (no week selection, locked to October 12-16)
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-3 p-4 bg-[#1a2d5a]/5 border-2 border-[#1a2d5a] rounded-lg">
        ...
        <div>
          <p className="font-semibold text-[#1a2d5a] text-sm">Fall Break Week, Fixed Date</p>
          <p className="text-sm text-gray-600">{FALL_BREAK_WEEK}</p>
        </div>
      </div>
...
function PaymentForm({ clientSecret, paymentIntentId, onSuccess }: { clientSecret: string; paymentIntentId: string; onSuccess: () => void }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const { error } = await stripe.confirmPayment({
      elements,
      confirmParams: { return_url: `${window.location.origin}/fall-break-registration` },
  ...
  if (paymentSuccess) {
    return (
      <div className="text-center py-12">
        ...
        <p className="text-gray-600 mb-2">Thank you for registering for TMA Fall Break Camp 2026!</p>
        <p className="text-gray-500 text-sm mb-2">A confirmation email has been sent to <strong>{data.email}</strong>.</p>
        <p className="text-gray-500 text-sm mb-6">We can't wait to see you October 12 to 16! 🥋</p>
...
// Main Fall Break Registration Page
export default function FallBreakRegistration() {
      ...
      <div className="bg-[#1a2d5a] text-white py-8">
        <div className="container max-w-3xl mx-auto px-4">
          <div className="flex items-center gap-3 mb-2">
            <button onClick={() => navigate("/fall-break-camp")} className="text-white/70 hover:text-white text-sm">← Fall Break Camp</button>
          </div>
          <h1 className="text-3xl font-bold">Fall Break Camp Registration</h1>
          <p className="text-white/80 mt-1">Top Martial Arts Suwanee • October 12 to 16, 2026 • 9:00 AM to 4:00 PM</p>
        </div>
      </div>
