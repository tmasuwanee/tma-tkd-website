...
import { Loader2, Users, DollarSign, CheckCircle, Clock, XCircle, Trash2, RotateCcw, Calendar, Eye, EyeOff, LogOut } from "lucide-react";
...
function StatusBadge({ status }: { status: string | null }) {
  if (status === "succeeded") {
    return <Badge className="bg-green-100 text-green-800 border-green-200 gap-1"><CheckCircle className="w-3 h-3" />Paid</Badge>;
  }
  if (status === "pending") {
    return <Badge className="bg-yellow-100 text-yellow-800 border-yellow-200 gap-1"><Clock className="w-3 h-3" />Pending</Badge>;
  }
    ...
    <div className="min-h-screen bg-gray-50 flex items-center justify-center px-4">
      <Card className="w-full max-w-sm shadow-lg">
        <CardHeader className="text-center pb-2">
          ...
          <p className="text-sm text-gray-500">Sign in to view camp registrations</p>
        </CardHeader>
        <CardContent>
...
export default function AdminRegistrations() {
  ...
  const { data: allRegistrations, isLoading, error } = trpc.admin.getCampRegistrations.useQuery(undefined, {
    enabled: isAuthed,
  });
  ...
  const softDelete = trpc.admin.softDeleteRegistration.useMutation({
    onSuccess: async () => {
      ...
      toast.success("Registration removed from active list.");
    },
  ...
  const paidCount = activeRegistrations.length;
  const totalCampers = activeRegistrations
    .reduce((sum, r) => sum + r.numCampers, 0);
  ...
  const handleBatchDelete = async () => {
    ...
    // Explicitly re-fetch after all deletes so stats update immediately
    await utils.admin.getCampRegistrations.invalidate();
    setSelectedIds(new Set());
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      {/* Header */}
      <div className="bg-[#1a2d5a] text-white px-6 py-5">
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-bold">Camp Registrations</h1>
            <p className="text-white/70 text-sm mt-1">Summer Camp 2026 — Admin View</p>
          </div>
          <div className="flex items-center gap-3">
            <a href="/" className="text-white/70 hover:text-white text-sm underline">← Back to Site</a>
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between gap-3 flex-wrap">
            ...
            <div className="flex items-center gap-2 flex-wrap">
              {/* Batch delete button — only shown when rows are selected and viewing active */}
                ...
                <Table>
                  <TableHeader>
                    <TableRow className="bg-gray-50">
                      {/* Select All checkbox — only shown for active view */}
                      {!showDeleted && (
                  ...
                  <TableBody>
                    {displayedRegistrations.map((reg) => {
                      ...
                      return (
                        <TableRow
                          key={reg.id}
                          className={`hover:bg-gray-50 ${reg.isDeleted ? "opacity-60" : ""} ${isChecked ? "bg-blue-50" : ""}`}
                        >
                          {/* Row checkbox — only shown for active view */}
                          {!showDeleted && (
                          ...
                          <TableCell>
                            <div className="space-y-0.5">
                              {campers.map((name, i) => (
                                <div key={i} className="text-sm font-medium text-gray-800">{name}</div>
                              ))}
                            </div>
                          </TableCell>
                          <TableCell>
                          ...
                          <TableCell>
                            <StatusBadge status={reg.stripePaymentStatus} />
                          </TableCell>
                          <TableCell>
                          ...
                          <TableCell>
                            {reg.isDeleted ? (
                              <Button
                                variant="ghost"
                                size="sm"
                                onClick={() => restore.mutate({ id: reg.id })}
      ...
      <AlertDialog open={deleteTargetId !== null} onOpenChange={(open) => { if (!open) setDeleteTargetId(null); }}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This will hide the registration from the active list. You can restore it anytime from the "View Deleted" section.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            ...
            <AlertDialogAction
              ...
              onClick={() => {
                if (deleteTargetId !== null) {
                  softDelete.mutate({ id: deleteTargetId });
      ...
      <AlertDialog open={showBatchDeleteConfirm} onOpenChange={setShowBatchDeleteConfirm}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              These registrations will be hidden from the active list. You can restore them anytime from the "View Deleted" section.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>Cancel</AlertDialogCancel>
